...
void send_receive_ASCLIN_UART_messagea(void)
{
    IfxAsclin_Asc_write(&g_ascHandle, ga_txData, &ga_count, TIME_INFINITE);   /* Transmit data via TX */
   // IfxAsclin_Asc_read(&g_ascHandle, g_rxData, &g_count, TIME_INFINITE);    /* Receive data via RX  */
}
...
void send_ASCLIN_UART_message(int AAA)
{
    g_rxData_b[0] = 174;
    g_rxData_b[1] = 184;
    g_rxData_b[2] = 'I';
    ...
    sprintf(AA,"%X",crc_16_val.b[0]);
    Ifx_SizeT g_count_a = sizeof(AA);
    IfxAsclin_Asc_write(&g_ascHandle,AA, &g_count_a, TIME_INFINITE);
    sprintf(BB,"%X",CC);
    Ifx_SizeT g_count_b = sizeof(BB);
    IfxAsclin_Asc_write(&g_ascHandle,BB, &g_count_b, TIME_INFINITE);
