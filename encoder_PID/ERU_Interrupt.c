...
void return_rpm(void){
    time = return_time();
    if(rpm_cnt<=450){
        timer_error=time;
    }
    else{
        timer_error=time - time_old;
    }
    rpm = 60000/(float)timer_error;
    time_old = time;
}

void SCUERU_Int0_Handler(void)
{
    if(IfxPort_getPinState(ENCODER_A_PIN) == 1 && IfxPort_getPinState(ENCODER_B_PIN) == 1){
        encoderPos++;
        rpm_cnt++;
    }

    else if(IfxPort_getPinState(ENCODER_A_PIN) == 1 && IfxPort_getPinState(ENCODER_B_PIN) == 0){
        encoderPos--;
        rpm_cnt++;
    }
